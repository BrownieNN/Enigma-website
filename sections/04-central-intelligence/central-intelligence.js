/* ENIGMA / 04 — Central Intelligence
   Extracted verbatim from js/droplets.js (grid-proof, branch enigma-buildout).

   DEPENDENCY: three.js (legacy UMD global — WebGLRenderer, RawShaderMaterial,
   PlaneGeometry, CanvasTexture). Load _global/global.js first, or add
   <script src="/js/three.min.js"> to the site template.

   Fonts: the shader draws the headline into a canvas texture using
   "Enigma Large" and "ABC Favorit Mono", and re-draws on document.fonts.ready.
   _global/global.css must be loaded or the type falls back to serif/monospace.

   NOT INCLUDED: js/droplets-panel.js — the gear-button art-direction panel
   (the control in the top right of the reference screenshot). It is dev
   chrome, not product code, and is deliberately left out of the CMS block.
   Live tuning is still available at runtime via window.__MOLTEN.set({...}).

   Changed for CMS use: the bootstrap at the foot of the file now waits for
   three.js instead of firing on DOMContentLoaded, so an async library load
   cannot race the init. Everything above it is untouched. */
...
(function () {
  ...
  /* Bootstrap: wait for the DOM *and* for three.js to exist. */
  function boot() {
    (window.enigmaReady || function (f) { f(); })(init);
  }
